<div class="px-5 py-4 border-t border-gray-200 flex flex-wrap items-center justify-between gap-3">
    ...
    <div class="flex items-center gap-2">
        @if($currentPage > 1)
            <a href="{{ route('jobs-monitor.dashboard', array_merge($extraParams, [$pageParam => $currentPage - 1])) }}"
        ...
        @if($start > 1)
            <a href="{{ route('jobs-monitor.dashboard', array_merge($extraParams, [$pageParam => 1])) }}"
        ...
        @for($p = $start; $p <= $end; $p++)
            <a href="{{ route('jobs-monitor.dashboard', array_merge($extraParams, [$pageParam => $p])) }}"
        ...
        @if($end < $lastPage)
            ...
            <a href="{{ route('jobs-monitor.dashboard', array_merge($extraParams, [$pageParam => $lastPage])) }}"
        ...
        <form method="GET" action="{{ route('jobs-monitor.dashboard') }}" class="flex items-center gap-1 ml-2">
