...
/**
 * JSON-only twin of FailureGroupsController.
 *
 * Mounted under the /api prefix so SPA / mobile / external integrations
 * can drive every page action without depending on the Blade UI. The
 * web-side controller exists only for traditional form posts (kebab
 * "Retry group" button) where a redirect-with-flash is the right UX.
 *
 * @internal
 */
final class FailureGroupsApiController extends Controller
{
    ...
    public function retry(string $fingerprint, BulkRetryDeadLetterAction $action): JsonResponse
    {
        ...
        return $this->bulkResponse($action($this->collectIdentifiers($group->fingerprint())));
    }

    public function destroy(string $fingerprint, BulkDeleteDeadLetterAction $action): JsonResponse
    {
    ...
    public function bulkRetry(Request $request, BulkRetryDeadLetterAction $action): JsonResponse
    {
        if (! $this->authorizeDestructive('retry')) {
            return new JsonResponse(['error' => 'Forbidden.'], 403);
        }
    ...
    public function bulkDelete(Request $request, BulkDeleteDeadLetterAction $action): JsonResponse
    {
        if (! $this->authorizeDestructive('delete')) {
            return new JsonResponse(['error' => 'Forbidden.'], 403);
        }
